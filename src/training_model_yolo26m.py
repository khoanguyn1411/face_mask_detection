...
DATASET_YAML = PROJECT_DIR / "datasets" / \
    "face-mask-detection-processed" / "dataset.yaml"
...
if IS_MAC:
    ...
    if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
        ...
        DEVICE_INFO = {
            "type": "Apple Silicon (Metal)", "is_gpu": True, "torch_device": "mps"}
    ...
    if torch.cuda.is_available():
        ...
        DEVICE_INFO = {
            "type": f"NVIDIA CUDA ({gpu_name})", "is_gpu": True, "torch_device": 0}
...
# Reduced from 256 for faster training (4x faster per iteration)
IMG_SIZE = 192
...
# Confidence threshold (lower = fewer detections, faster NMS)
CONF_THRESHOLD = 0.45
# IOU threshold for NMS (lower = fewer overlapping boxes)
IOU_THRESHOLD = 0.45
MAX_DETECTIONS = 300   # Maximum detections per image (reduces NMS workload)

BASE_NAME = "face_mask_detection_yolo26m_v1"
PREDICTION_NAME = "predictions_yolo26m_v1"
    ...
    if IS_MAC:
        ...
        if torch.cuda.is_available():
            ...
            if total_memory <= 4.5:
                ...
                if BATCH_SIZE > 4:
                    print(
                        f"    • If OOM errors: Reduce BATCH_SIZE to {max(2, BATCH_SIZE//2)}")
                print(f"    • FP16 Training: ENABLED (saves ~50% memory)")
                if IMG_SIZE > 320:
                    print(
                        f"    • Option: Reduce IMG_SIZE to 320 (currently {IMG_SIZE})")
...
def train_model():
    """Train YOLOv26m model on face mask detection dataset."""
    print("=" * 80)
    print("🎯 YOLOv26m Training - Face Mask Detection")
    print("=" * 80)
    ...
    # Load or create YOLOv26m model
    print("\n" + "=" * 80)
    print("📥 Loading YOLOv26m model...")
    print("=" * 80)

    try:
        model = YOLO("yolo26m.pt")  # Load pretrained YOLOv26m
        print("✓ YOLOv26m model loaded successfully")
    ...
    try:
        results = model.train(
            ...
            # Logging parameters
            plots=True,
            # Use FP16 if GPU available (CUDA or MPS)
            half=DEVICE_INFO['is_gpu'],
        )
        ...
        print("\n📈 Training Results:")
        print(
            f"  Best model path: {model.trainer.best.name if hasattr(model, 'trainer') else 'See runs directory'}")
