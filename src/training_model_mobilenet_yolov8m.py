...
def load_images_from_split(image_dir, labels_dir, yolo_model):
    """Load images and extract face crops using YOLOv8m detection instead of ground truth."""
    images = []
    classes = []

    print(f"  Using YOLOv8m for face detection (not ground truth)...")
    ...
    for idx, img_path in enumerate(image_files):
        ...
        try:
            ...
            # Run YOLOv8m inference on image
...
def prepare_dataset():
    """Load preprocessed dataset and extract face crops using YOLOv8m detection."""
    print("=" * 80)
    print("📊 PREPARING DATASET - Extracting Face Crops with YOLOv8m Detection")
    print("   (Using YOLOv8m for face detection instead of ground truth annotations)")
    print("=" * 80)
    ...
    # Load YOLOv8m model for face detection
    print("\n📥 Loading YOLOv8m model for face detection...")
    try:
        PROJECT_DIR = Path(__file__).parent.parent
        model_path = PROJECT_DIR / "runs" / \
            "face_mask_detection_yolov8m_v2" / "weights" / "best.pt"
        ...
        if not model_path.exists():
            ...
            print(f"   ✓ Using default YOLOv8m model")
        else:
            yolo_model = YOLO(str(model_path))
            print(f"   ✓ Loaded trained YOLOv8m model from {model_path}")
    except Exception as e:
        print(f"❌ Error loading YOLOv8m model: {e}")
        return None

    # Load train split
    print(f"\nExtracting train split face crops using YOLOv8m detection...")
    X_train, y_train = load_images_from_split(
        IMAGES_TRAIN_DIR, LABELS_TRAIN_DIR, yolo_model)

    # Load validation split
    print(f"Extracting validation split face crops using YOLOv8m detection...")
    X_val, y_val = load_images_from_split(
        IMAGES_VAL_DIR, LABELS_VAL_DIR, yolo_model)

    # Load test split
    print(f"Extracting test split face crops using YOLOv8m detection...")
    X_test, y_test = load_images_from_split(
        IMAGES_TEST_DIR, LABELS_TEST_DIR, yolo_model)

    print(f"\n✓ Face Crops Extracted Successfully with YOLOv8m")
...
def save_model(model):
    ...
    model_path = MODELS_DIR / "face_mask_mobilenet_yolov8m_best.h5"
...
def main():
    ...
    print("   Face Detection: YOLOv8m (not ground truth annotations)")
